...
import { HiMenuAlt2 } from 'react-icons/hi'; // Importing the menu icon from react-icons
import { FaUserCircle } from 'react-icons/fa'; // Importing the user profile icon

const Navbar = ({ onMenuToggle, isSidebarOpen }) => {
  return (
    <header
      className={`bg-white-800 text-gray p-4 flex justify-between items-center transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center space-x-4">
        <button
          ...
          className="text-2xl p-2 rounded-md hover:bg-gray-700"
        >
          <HiMenuAlt2 />
        </button>
